...
if (!fs.existsSync(templatePath)) {
  fail("dist/index.html not found. Did the client build run (vite build)?");
}
const serverEntryRel = "./dist/server/entry-server.js";
const serverEntryPath = toAbs(serverEntryRel);
if (!fs.existsSync(serverEntryPath)) {
  fail("dist/server/entry-server.js not found. Did the server build run?");
}

// 2) Import template and server renderer
const template = fs.readFileSync(templatePath, "utf-8");
const { render } = await import(serverEntryRel);
if (typeof render !== "function") {
  fail("Named export `render` not found in dist/server/entry-server.js");
}

// 3) Your explicit route list
...
// 4) Render each route with explicit error reporting
let okCount = 0;
...
log(`Successfully pre-rendered ${okCount}/${routesToPrerender.length} routes.`);
